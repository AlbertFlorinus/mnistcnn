...
def show_result(test_img, test_img_ae, test_img2, test_img2_ae, filename):
    ...
    fig.add_subplot(rows, columns, 1).set_title("adaptive_Threshold")
    plt.imshow(test_img)
    fig.add_subplot(rows, columns, 2).set_title("adaptive_Threshold+downscale")
    plt.imshow(test_img_ae)
    #fig.add_subplot(rows, columns, 2).set_title("Threshold\n j = {}".format(j))
    fig.add_subplot(rows, columns, 3).set_title("Threshold")
    plt.imshow(test_img2)
    fig.add_subplot(rows, columns, 4).set_title("adaptive_Threshold+downscale")
    plt.imshow(test_img2_ae)

    fig.add_subplot(rows, columns, 5).set_title("Orig img")
    orig = cv2.imread(f"digits/{filename}", 0)
    plt.imshow(orig)
    fig.suptitle(f"Image before preprocessing")
...
if __name__ == "__main__":
    ...
    for filename in os.listdir("digits"):
        ...
        if filename.endswith((".DS_Store")):
            ...
            #pred1, pred2, test_img, gray_mini = decide(model, filename)
            #resize_img = resizer_model.predict(test_img)
            #classname = weighted_class(pred1, pred2)
            #show_result(img1, img2, classname, filename)
            show_result(img1, img1_ae, img2, img2_ae, filename)
            count += 1