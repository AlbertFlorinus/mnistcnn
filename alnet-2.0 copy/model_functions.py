...
class Run():
	...
	def predict_folder_orig(self, pather):
		
		"""for root, dirs, files in os.walk(pather, topdown=False):
			for name in files:
		...
		for root, dirs, files in os.walk(pather, topdown=False):
			...
			if 1 == 2:
				...
				gray = cv2.imread(pather + "/%s"%dirs + "/%s"%filename, 0)
				gray = cv2.resize(255-gray, (112, 112), interpolation=cv2.INTER_AREA)
				#(thresh, gray) = cv2.threshold(gray, 128, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
	...
	def predict_folder(self, pather):
		...
		for folders in dirs:
			...
			if fold.endswith((".DS_Store")):
				...
				print(fold)
				for i in os.listdir("digits/"+fold):
